...
- [ ] Implement mobile menu focus trap in the inline `<script>` block of `index.html`:
...
- [ ] Perform semantic HTML cleanup in `index.html`:
...
- [ ] Define local font fallback stacks in `css/design-tokens.css`:
...
- [ ] Create WebP versions of the PNG logo and update HTML references:
...
- [ ] Add cache-busting query strings to static asset references in `index.html`:
...
- [ ] Activate dark mode via `prefers-color-scheme` media query:
...
- [ ] Verify Phase 02 changes are correct and nothing is broken:
